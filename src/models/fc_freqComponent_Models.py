...
from tf_utils import weight_variable, bias_variable , dense_layer


def build_fc_freq_5_BN_Tiny(
        x, x_dim, 
        keep_prob, 
        is_training, 
        gamma=1e-7, 
    ...
    # FC1
    fc1_dim = 500
    h = dense_layer(x, [x_dim, fc1_dim], "FC1",
            activation='elu',
    ...
    #FC3
    fc3_dim = 100
    h = dense_layer(h, [fc2_dim, fc3_dim], "FC3",
            activation='elu',
            use_bn=True, bn_first=True,
            is_training=is_training)
    # dropout
    ...
    with tf.name_scope("FCFeat"):
        ...
        h = tf.nn.elu(tf.matmul(h, W_fc4) + b_fc4, name="feature")

    #FC5
    fc5_dim = 100
    h = dense_layer(h, [fc4_dim, fc5_dim], "FC5",
            activation='elu',
            use_bn=True, bn_first=True,
            is_training=is_training)
    # dropout
    ...
    #FC6
    fc6_dim = 300
    h = dense_layer(h, [fc5_dim, fc6_dim], "FC6",
            activation='elu',
            use_bn=True, bn_first=True,
            is_training=is_training)
    # dropout
...
def build_fc_freq_5_NoTiedWeight_L1_Small(
        x, x_dim, 
        keep_prob, 
        is_training, 
        gamma=1e-7, 
