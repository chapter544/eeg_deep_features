...
def build_fc_freqSum_NoTiedWeight_BN_Wrapper_Behind_Tiny(
        x, 
        x_dim, 
        keep_prob, 
        is_training,
    ...
    with tf.name_scope("FC1"):
        ...
        h_fc1_bn = tf.nn.elu(tf.matmul(x, W_fc1) + b_fc1)
    ...
    with tf.name_scope("FC2"):
        ...
        h_fc2_bn = tf.nn.elu(tf.matmul(h_fc1_bn, W_fc2) + b_fc2)
        #h_fc2_bn = batch_norm_wrapper(h_fc2, is_training)

    with tf.name_scope("FC2a"):
        fc2a_dim = 300
        W_fc2a = weight_variable([fc2_dim, fc2a_dim])
        b_fc2a = weight_variable([fc2a_dim])
        #h_fc2 = tf.nn.elu(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)
        h_fc2a_bn = tf.nn.elu(tf.matmul(h_fc2_bn, W_fc2a) + b_fc2a)
        #h_fc2a_bn = batch_norm_wrapper(h_fc2a, is_training)


    # dropout
    #with tf.name_scope("Dropout2"):
    #    h_fc2_drop = tf.nn.dropout(h_fc2a_bn, keep_prob)

    # FC3
    with tf.name_scope("FC3"):
        fc3_dim = 200
        W_fc3 = weight_variable([fc2a_dim, fc3_dim])
        b_fc3 = weight_variable([fc3_dim])
        #h_fc3 = tf.nn.elu(tf.matmul(h_fc2_drop, W_fc3) + b_fc3)
        h_fc3_bn = tf.nn.elu(tf.matmul(h_fc2a_bn, W_fc3) + b_fc3)
    ...
    with tf.name_scope("FC5"):
        ...
        h_fc5_bn = tf.nn.elu(tf.matmul(h_fc4, W_fc5) + b_fc5)
    ...
    with tf.name_scope("FC6"):
        ...
        h_fc6_bn = tf.nn.elu(tf.matmul(h_fc5_bn, W_fc6) + b_fc6)
        #h_fc6_bn = batch_norm_wrapper(h_fc6, is_training)

    with tf.name_scope("FC6a"):
        fc6a_dim = 600
        W_fc6a = weight_variable([fc6_dim, fc6a_dim])
        b_fc6a = weight_variable([fc6a_dim])
        h_fc6a_bn = tf.nn.elu(tf.matmul(h_fc6_bn, W_fc6a) + b_fc6a)
        #h_fc6a_bn = batch_norm_wrapper(h_fc6a, is_training)
    ...
    with tf.name_scope("FC7"):
        fc7_dim = 800
        W_fc7 = weight_variable([fc6a_dim, fc7_dim])
        b_fc7 = weight_variable([fc7_dim])
        #h_fc7 = tf.nn.elu(tf.matmul(h_fc6_drop, W_fc7) + b_fc7)
        h_fc7_bn = tf.nn.elu(tf.matmul(h_fc6a_bn, W_fc7) + b_fc7)
...
def build_fc_freq_5_NoTiedWeight_Behind_Tiny(
        x, 
        x_dim, 
        keep_prob, 
        is_training,
    ...
    with tf.name_scope("FC2a"):
        ...
        W_fc2a = weight_variable([fc2_dim, fc2a_dim])
        b_fc2a = weight_variable([fc2a_dim])
        h_fc2a = tf.nn.elu(tf.matmul(h_fc2, W_fc2a) + b_fc2a)


    # dropout
    #with tf.name_scope("Dropout2"):
    #    h_fc2a = tf.nn.dropout(h_fc2a, keep_prob)
    ...
    with tf.name_scope("FC3"):
        ...
        W_fc3 = weight_variable([fc2a_dim, fc3_dim])
        b_fc3 = weight_variable([fc3_dim])
        h_fc3 = tf.nn.elu(tf.matmul(h_fc2a, W_fc3) + b_fc3)
    ...
    with tf.name_scope("FC6a"):
        ...
        W_fc6a = weight_variable([fc6_dim, fc6a_dim])
        b_fc6a = weight_variable([fc6a_dim])
        h_fc6a = tf.nn.elu(tf.matmul(h_fc6, W_fc6a) + b_fc6a)


    # dropout
    with tf.name_scope("Dropout6"):
        h_fc6a = tf.nn.dropout(h_fc6, keep_prob)

    with tf.name_scope("FC7"):
        fc7_dim = 400
        W_fc7 = weight_variable([fc6a_dim, fc7_dim])
        b_fc7 = weight_variable([fc7_dim])
        h_fc7 = tf.nn.elu(tf.matmul(h_fc6a, W_fc7) + b_fc7)
...
def build_fc_freq_5_NoTiedWeight_BN_Wrapper_Behind_Tiny(
        x, 
        x_dim, 
        keep_prob, 
        is_training,
    ...
    with tf.name_scope("FC2a"):
        ...
        b_fc2a = weight_variable([fc2a_dim])
        #h_fc2 = tf.nn.elu(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)
        h_fc2a = tf.nn.elu(tf.matmul(h_fc2_bn, W_fc2a) + b_fc2a)
        h_fc2a_bn = batch_norm_wrapper(h_fc2a, is_training)
    ...
    with tf.name_scope("FC3"):
        ...
        b_fc3 = weight_variable([fc3_dim])
        #h_fc3 = tf.nn.elu(tf.matmul(h_fc2_drop, W_fc3) + b_fc3)
        h_fc3 = tf.nn.elu(tf.matmul(h_fc2a_bn, W_fc3) + b_fc3)
        h_fc3_bn = batch_norm_wrapper(h_fc3, is_training)
    ...
    with tf.name_scope("FC6a"):
        ...
        W_fc6a = weight_variable([fc6_dim, fc6a_dim])
        b_fc6a = weight_variable([fc6a_dim])
        h_fc6a = tf.nn.elu(tf.matmul(h_fc6_bn, W_fc6a) + b_fc6a)
        h_fc6a_bn = batch_norm_wrapper(h_fc6a, is_training)
    ...
    with tf.name_scope("FC7"):
        ...
        b_fc7 = weight_variable([fc7_dim])
        #h_fc7 = tf.nn.elu(tf.matmul(h_fc6_drop, W_fc7) + b_fc7)
        h_fc7 = tf.nn.elu(tf.matmul(h_fc6a_bn, W_fc7) + b_fc7)
        h_fc7_bn = batch_norm_wrapper(h_fc7, is_training)
...
def build_fc_freq_4_30_NoTiedWeight_BN_Wrapper_Behind_Tiny(
        x, 
        x_dim, 
        keep_prob, 
        is_training,
...
def build_fc_freq_4_30_NoTiedWeight_BN_Wrapper_Tiny(
        x, 
        x_dim, 
        keep_prob, 
        is_training,
    ...
    with tf.name_scope("FC1"):
        ...
        h_fc1_bn = tf.nn.elu(batch_norm_wrapper(h_fc1, is_training))
    ...
    with tf.name_scope("FC2"):
        ...
        #h_fc2 = tf.matmul(h_fc1_bn, W_fc2) + b_fc2
        h_fc2_bn = tf.nn.elu(batch_norm_wrapper(h_fc2, is_training))
    ...
    with tf.name_scope("FC3"):
        ...
        #h_fc3 = tf.matmul(h_fc2_bn, W_fc3) + b_fc3
        h_fc3_bn = tf.nn.elu(batch_norm_wrapper(h_fc3, is_training))
    ...
    with tf.name_scope("FCFeat"):
        ...
        W_fc4 = weight_variable([fc3_dim, fc4_dim])
        b_fc4 = weight_variable([fc4_dim])
        h_fc4 = tf.matmul(h_fc3_bn, W_fc4) + b_fc4
        h_fc4_bn = tf.nn.elu(
                          batch_norm_wrapper(h_fc4, is_training), 
                          name="feature")
    ...
    with tf.name_scope("FC5"):
        ...
        W_fc5 = weight_variable([fc4_dim, fc5_dim])
        b_fc5 = weight_variable([fc5_dim])
        h_fc5 = tf.matmul(h_fc4_bn, W_fc5) + b_fc5
        h_fc5_bn = tf.nn.elu(batch_norm_wrapper(h_fc5, is_training))
    ...
    with tf.name_scope("FC6"):
        ...
        b_fc6 = weight_variable([fc6_dim])
        h_fc6 = tf.matmul(h_fc5_drop, W_fc6) + b_fc6
        h_fc6_bn = tf.nn.elu(batch_norm_contrib(h_fc6, is_training))
...
def build_fc_freq_4_30_NoTiedWeight_BN_Contrib_Tiny(
        x, 
        x_dim, 
        keep_prob, 
        is_training,
    ...
    with tf.name_scope("FCFeat"):
        ...
        b_fc4 = weight_variable([fc4_dim])
        h_fc4 = tf.matmul(h_fc3_bn, W_fc4) + b_fc4
        h_fc4 = tf.nn.elu(h_fc4, name="feature")
    ...
    with tf.name_scope("FC6"):
        ...
        b_fc6 = weight_variable([fc6_dim])
        h_fc6 = tf.matmul(h_fc5_drop, W_fc6) + b_fc6
        #h_fc6 = tf.matmul(h_fc5_bn, W_fc6) + b_fc6
        h_fc6_bn = tf.nn.elu(batch_norm_contrib(h_fc6, is_training))
    ...
    with tf.name_scope("FC7"):
        ...
        W_fc7 = weight_variable([fc6_dim, fc7_dim])
        b_fc7 = weight_variable([fc7_dim])
        h_fc7 = tf.matmul(h_fc6_, W_fc7) + b_fc7
        h_fc7_bn = tf.nn.elu(batch_norm_contrib(h_fc7, is_training))
...
def build_fc_freq_5_NoTiedWeight_BN_Wrapper_Behind_Small (
        x, x_dim, 
        keep_prob, 
        is_training, 
        gamma=1e-7, 
    ...
    with tf.name_scope("FC1"):
        ...
        W_fc1 = weight_variable([x_dim, fc1_dim])
        b_fc1 = weight_variable([fc1_dim])
        h_fc1_bn = batch_norm_wrapper(
                    tf.nn.elu(tf.matmul(x, W_fc1) + b_fc1),
                    is_training)
    ...
    with tf.name_scope("FC2"):
        ...
        W_fc2 = weight_variable([fc1_dim, fc2_dim])
        b_fc2 = weight_variable([fc2_dim])
        h_fc2_bn = batch_norm_wrapper(
                    #tf.nn.elu(tf.matmul(h_fc1_drop, W_fc2) + b_fc2),
                    tf.nn.elu(tf.matmul(h_fc1_bn, W_fc2) + b_fc2),
                    is_training)
    # dropout
    with tf.name_scope("Dropout2"):
        h_fc2_drop = tf.nn.dropout(h_fc2_bn, keep_prob)
    ...
    with tf.name_scope("FC3"):
        ...
        W_fc3 = weight_variable([fc2_dim, fc3_dim])
        b_fc3 = weight_variable([fc3_dim])
        h_fc3_bn = batch_norm_wrapper(
                    tf.nn.elu(tf.matmul(h_fc2_drop, W_fc3) + b_fc3),
                    is_training)
    ...
    with tf.name_scope("FCFeat"):
        ...
        W_fc4 = weight_variable([fc3_dim, fc4_dim])
        b_fc4 = weight_variable([fc4_dim])
        h_fc4 = batch_norm_wrapper(
                    tf.matmul(h_fc3_bn, W_fc4) + b_fc4, is_training)
        h_fc4_bn = tf.nn.elu(h_fc4, name="feature")
    ...
    with tf.name_scope("FC5"):
        ...
        W_fc5 = weight_variable([fc4_dim, fc5_dim])
        b_fc5 = weight_variable([fc5_dim])
        h_fc5_bn = batch_norm_wrapper(
                    tf.nn.elu(tf.matmul(h_fc4_bn, W_fc5) + b_fc5),
                    is_training)
    ...
    with tf.name_scope("FC6"):
        ...
        W_fc6 = weight_variable([fc5_dim, fc6_dim])
        b_fc6 = weight_variable([fc6_dim])
        h_fc6_bn = batch_norm_wrapper(
                    tf.nn.elu(tf.matmul(h_fc5_drop, W_fc6) + b_fc6),
                    is_training)
    ...
    with tf.name_scope("FC7"):
        ...
        W_fc7 = weight_variable([fc6_dim, fc7_dim])
        b_fc7 = weight_variable([fc7_dim])
        h_fc7_bn = batch_norm_wrapper(
                    #tf.nn.elu(tf.matmul(h_fc6_drop, W_fc7) + b_fc7),
                    tf.nn.elu(tf.matmul(h_fc6_bn, W_fc7) + b_fc7),
                    is_training)
...
def build_fc_freq_5_NoTiedWeight_BN_Wrapper_Tiny (
        x, x_dim, 
        keep_prob, 
        is_training, 
        gamma=1e-7, 
    ...
    with tf.name_scope("FCFeat"):
        ...
        W_fc4 = weight_variable([fc3_dim, fc4_dim])
        b_fc4 = weight_variable([fc4_dim])
        h_fc4_bn = batch_norm_wrapper(
                    tf.nn.elu(
                        tf.matmul(h_fc3_bn, W_fc4) + b_fc4,
                        name="feature"), is_training)
...
def build_fc_freq_5_NoTiedWeight_BN_Contrib_Behind_Tiny (
        x, x_dim, 
        keep_prob, 
        is_training, 
        gamma=1e-7, 
...
def build_fc_freq_5_NoTiedWeight_BN_Contrib_Tiny (
        x, x_dim, 
        keep_prob, 
        is_training, 
        gamma=1e-7, 
