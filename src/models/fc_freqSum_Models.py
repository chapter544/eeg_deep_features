...
def build_fc_freqSum_TiedWeight_NoBias(x, x_dim, keep_prob, gamma=1e-7):
    ...
    with tf.name_scope("FC1"):
        ...
        h_fc1 = tf.nn.elu(tf.matmul(x, W_fc1))
    ...
    with tf.name_scope("FC2"):
        ...
        h_fc2 = tf.nn.elu(tf.matmul(h_fc1, W_fc2))
    ...
    with tf.name_scope("FC3"):
        ...
        h_fc3 = tf.nn.elu(tf.matmul(h_fc2_drop, W_fc3))
    ...
    with tf.name_scope("FC4"):
        ...
        h_fc4 = tf.nn.elu(tf.matmul(h_fc3, W_fc4))

    # FC5
    with tf.name_scope("FC5"):
        fc5_dim = 256
        W_fc5 = weight_variable([fc4_dim, fc5_dim])
        h_fc5 = tf.nn.elu(tf.matmul(h_fc4, W_fc5))

    # FC6
    with tf.name_scope("FCFeat"):
        fc6_dim = 64
        W_fc6 = weight_variable([fc5_dim, fc6_dim])
        h_fc6 = tf.nn.elu(tf.matmul(h_fc5, W_fc6), name="feature")
    ...
    with tf.name_scope("FC7"):
        ...
        h_fc7 = tf.nn.elu(tf.matmul(h_fc6, W_fc7))

    # FC8
    with tf.name_scope("FC8"):
        fc8_dim = 512
        W_fc8 = tf.transpose(W_fc5)
        h_fc8 = tf.nn.elu(tf.matmul(h_fc7, W_fc8))

    # FC9
    with tf.name_scope("FC9"):
        fc9_dim = 1024
        W_fc9 = tf.transpose(W_fc4)
        h_fc9 = tf.nn.elu(tf.matmul(h_fc8, W_fc9))
    ...
    with tf.name_scope("FC10"):
        ...
        h_fc10 = tf.nn.elu(tf.matmul(h_fc9, W_fc10))
    ...
    with tf.name_scope("FC11"):
        ...
        h_fc11 = tf.nn.elu(tf.matmul(h_fc10_drop, W_fc11))
...
def build_fc_freqSum_TiedWeight_VeryBig(x, x_dim, keep_prob, gamma=1e-7):
    # FC1
    with tf.name_scope("FC1"):
        fc1_dim = 8192
        W_fc1 = weight_variable([x_dim, fc1_dim])
    ...
    with tf.name_scope("FCFeat"):
        ...
        h_fc6 = tf.nn.elu(tf.matmul(h_fc5, W_fc6) + b_fc6, name="feature")
...
def build_fc_freqSum_NoTiedWeight_BN_Small(
        x, x_dim, 
        keep_prob, 
        is_training,
        gamma=1e-7,
...
def build_fc_freqSum_NoTiedWeight_Big(x, x_dim, keep_prob, gamma=1e-7,
    ...
    with tf.name_scope("FC8"):
        ...
        W_fc8 = weight_variable([fc7_dim, fc8_dim])
        b_fc8 = weight_variable([fc8_dim])
        #h_fc8 = tf.nn.elu(tf.matmul(h_fc7_drop, W_fc8) + b_fc8)
        h_fc8 = tf.nn.elu(tf.matmul(h_fc7, W_fc8) + b_fc8)
    ...
    with tf.name_scope("FC9"):
        ...
        b_fc9 = weight_variable([fc9_dim])
        h_fc9 = tf.nn.elu(tf.matmul(h_fc8, W_fc9) + b_fc9)
        #h_fc9 = tf.nn.elu(tf.matmul(h_fc8_drop, W_fc9) + b_fc9)
    ...
    with tf.name_scope("FC10"):
        ...
        b_fc10 = weight_variable([fc10_dim])
        #h_fc10 = tf.nn.elu(tf.matmul(h_fc9_drop, W_fc10) + b_fc10)
        h_fc10 = tf.nn.elu(tf.matmul(h_fc9, W_fc10) + b_fc10)
    ...
    with tf.name_scope("FC11"):
        ...
        b_fc11 = weight_variable([fc11_dim])
        #h_fc11 = tf.nn.elu(tf.matmul(h_fc10_drop, W_fc11) + b_fc11)
        h_fc11 = tf.nn.elu(tf.matmul(h_fc10, W_fc11) + b_fc11)
#
#    # FC12
    with tf.name_scope("FC12"):
        fc12_dim = x_dim
...
def build_fc_freqSum_TiedWeight(x, x_dim, keep_prob, gamma=1e-7):
    ...
    with tf.name_scope("FC12"):
        ...
        W_fc12 = tf.transpose(W_fc1)
        b_fc12 = weight_variable([fc12_dim])
        #y = tf.nn.elu(tf.matmul(h_fc11, W_fc12) + b_fc12)
        y = tf.matmul(h_fc11, W_fc12) + b_fc12
